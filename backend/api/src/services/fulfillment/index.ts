...
import { VendorPortalProvider } from './portal.provider';

export function getFulfillmentProvider(type: 'EMAIL' | 'API' | 'DIGITAL' | 'PORTAL'): FulfillmentProvider {
  switch (type) {
    case 'EMAIL':
    ...
    case 'PORTAL':
      return new VendorPortalProvider();
    case 'API':
